...
@implementation AudioEqualizerViewController
{
...
- (void)viewDidLoad {
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"百战成诗" ofType:@"mp3"];
...
- (void)setSubViews {
    ...
    for (int i = 0; i < 11; i++) {
        ...
        UISlider *slider = [[UISlider alloc] initWithFrame:CGRectMake(50, 88 + margin*(i+1)+20*i, 200, 20)];
        slider.tag = 1000 + i;
...
- (void)setupEqFilter:(AEParametricEqFilter *)eqFilter centerFrequency:(double)centerFrequency gain:(double)gain {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ( ![self.audioController.filters containsObject:eqFilter] ) {
            for (AEParametricEqFilter *existEqFilter in self->_eqFilters) {
