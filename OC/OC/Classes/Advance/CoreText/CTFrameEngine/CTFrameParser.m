...
+ (NSDictionary *)attributesWithConfig:(CTFrameParserConfig *)config{
    ...
    const CFIndex kNumberOffsetting = 3;
    CTParagraphStyleSetting theSetting[kNumberOffsetting] = {
        { kCTParagraphStyleSpecifierLineSpacingAdjustment, sizeof(CGFloat), &lineSpacing},
        { kCTParagraphStyleSpecifierMaximumLineSpacing, sizeof(CGFloat), &lineSpacing},
        { kCTParagraphStyleSpecifierMinimumLineSpacing, sizeof(CGFloat), &lineSpacing},
    };
    
    CTParagraphStyleRef theParagraphRef = CTParagraphStyleCreate(theSetting, kNumberOffsetting);
...
+ (CoreTextData *)parseAttributedContent:(NSAttributedString *)content config:(CTFrameParserConfig *)config{
    ...
    //将生成好的CTFrameRef实例和计算好的缓存高度保存到CoreTextData
    CoreTextData *data = [[CoreTextData alloc] init];
    data.ctFrame = frame;
    data.height = textHeight;
